...
class Database:
    ...
    def get_gender(name: str) -> int:
        ...
        try:
            result = session.query(Genders).\
                filter(Genders.name == name).one().sex
    ...
    def get_user_info(user_id: int) -> dict:
        ...
        result = session.query(Users).\
            filter(Users.id_user == user_id).one()
    ...
    def upload_partner_info(user_id: int, partner_info: dict, ignore=False):
        ...
        model.users_partners = [UsersPartners(id_user=user_id,
                                              ignore=ignore)]
        session.add(model)
        session.commit()
    ...
    def check_ignore(user_id: int, partner_id: int) -> bool:
        ...
        '''
        try:
            result = session.query(UsersPartners).\
                filter(UsersPartners.id_user == user_id,
                       UsersPartners.id_partner == partner_id).one().ignore
        except sq.exc.NoResultFound:
            result = False
        return result
    ...
    def check_prkey_in_partners(partner_id: int) -> bool:
        ...
        try:
            result = session.query(Partners).\
                filter(Partners.id_partner == partner_id).one()
        except sq.exc.NoResultFound:
            result = False
        return bool(result)
    ...
    def check_prkey_in_users_partners(user_id: int, partner_id: int) -> bool:
        ...
        try:
            result = session.query(UsersPartners).\
                filter(UsersPartners.id_user == user_id,
                       UsersPartners.id_partner == partner_id).one()
        except sq.exc.NoResultFound:
            result = False
        return bool(result)
...
class VkontakteBot(VkontakteAPI):
    ...
    def show_found_people(self, user_id: int):
        '''Метод отправки в чат пользователю найденного партнера.
           ...
           Выполняется при получении команды 'Далее' при условии что ранее 
           выполнен поиск всех подходящих партнеров find_all_partners или 
           вызывается из функции start_searching_handling.
    ...
    def start_handling(self, event: Event):
        ...
        match event.text:
            ...
            case _:
                self.send_message(event.user_id, 'Такой команды не знаю! \U0001F937')
